export const generateContent = async (prompt, b64Image = null, useMcp = false) => {
    ...
    var response;
    // Use MCP or Gemini based on the flag
    if (useMcp) { // Use MCP for chat
        response = await fetch('/api/gemini/mcp', {
            method: 'POST',
            headers: {
    ...
    } else { // Fallback to Gemini without MCP
